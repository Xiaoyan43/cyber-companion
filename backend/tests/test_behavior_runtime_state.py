...
from backend.app.memory.schema import OPERATIONAL_MOOD_METADATA_KEYS
...
def _operational_fixture() -> dict[str, object]:
    return {
        ...
        "last_proactive_fired_at": "2026-06-20T01:01:00+00:00",
        "proactive_pending_since": "2026-06-20T01:01:00+00:00",
        "proactive_daily_date": "2026-06-20",
        "proactive_daily_count": 2,
        "last_local_line_at": "2026-06-20T01:01:01+00:00",
        "proactive_llm_daily_date": "2026-06-20",
        "proactive_llm_daily_count": 1,
...
def test_canonical_runtime_wins_while_mood_view_preserves_local_keys(tmp_path: Path) -> None:
    ...
    store.patch_behavior_runtime_metadata(updates={"proactive_daily_count": 4})
    with connect(db_path) as connection:
        legacy = loads_json(
            connection.execute(
        ...
        legacy["proactive_daily_count"] = 999
        connection.execute(
    ...
    assert merged["proactive_daily_count"] == 4
...
def test_runtime_patch_and_remove_dual_write_legacy_copy(tmp_path: Path) -> None:
    ...
    runtime = store.patch_behavior_runtime_metadata(
        ...
        remove=("proactive_pending_since",),
    )
...
def test_local_metadata_patch_does_not_overwrite_runtime(tmp_path: Path) -> None:
    ...
    store.patch_behavior_runtime_metadata(updates={"proactive_daily_count": 3})
    store.patch_mood_metadata(updates={"positive_zone_streak": 2})

    assert store.get_behavior_runtime_state().metadata == {"proactive_daily_count": 3}
