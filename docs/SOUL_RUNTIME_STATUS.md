...
| **current HEAD** | `137b5d4` — `docs(soul): set accepted HEAD to f844ed5 for Phase 6` |
| **accepted HEAD** | `137b5d4` — Phase 0–6 全部验收通过；下一任务从 Phase 7 起 |
...
## 2. 测试基线（accepted @ `137b5d4` / feat `e0b4f23`）
...
| **6** | ✅ accepted | `e0b4f23` · `137b5d4` | proactive 迁 `motivation.py`：`agenda` 模式 due/overdue open_loop 等为 reason 来源；longing/Poisson 仅节奏闸；`proactive_reason_mode=longing_only` rollback |
...
| Phase | 范围 | 风险 | 前置 |
|---|---|---|---|
| **6** | proactive 迁 agenda/motivation；longing/Poisson 仅作节奏闸 | 中 | ✅ accepted @ `137b5d4` |
...
四条路线共享 **invariant 358** 回归门禁；语音轨额外遵守 voice 专项测试，但不反向改动 Soul Runtime 契约。
