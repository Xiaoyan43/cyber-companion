...
| **current HEAD** | `ab85ae1` — `docs(soul): mark Phase 6 accepted @ e0b4f23` |
| **accepted HEAD** | `ab85ae1` — Phase 0–6 全部验收通过；下一任务从 Phase 7 起 |
...
## 2. 测试基线（accepted @ `ab85ae1` / feat `e0b4f23`）
...
| **6** | ✅ accepted | `e0b4f23` · `ab85ae1` | proactive 迁 `motivation.py`：`agenda` 模式 due/overdue open_loop 等为 reason 来源；longing/Poisson 仅节奏闸；`proactive_reason_mode=longing_only` rollback |
...
| Phase | 范围 | 风险 | 前置 |
|---|---|---|---|
| **6** | proactive 迁 agenda/motivation；longing/Poisson 仅作节奏闸 | 中 | ✅ accepted @ `ab85ae1` |
