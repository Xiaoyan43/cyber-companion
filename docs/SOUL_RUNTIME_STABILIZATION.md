...
| 开关 | 位置 | 默认值（example） | 作用 |
|---|---|---|---|
| `proactive_reason_mode` | `config/budget.json` | `"agenda"` | `"longing_only"` = Phase 6 前 proactive 行为 |
| `enable_proactive` | `config/budget.json` | `true` | 关闭全部 proactive |
| `enable_turn_analyzer` | `config/budget.json` | `true` | 关闭 RTC off-path `analyze_turn` |
| `analyze_every_n_turns` | `config/budget.json` | `1` | 增大 = 降低 off-path LLM 分析频率 |
| `enable_reflection` | `config/budget.json` | `true` | off-path reflection worker |
| 空 `open_loops` 表 | SQLite | — | 天然 kill switch：agenda mode 无 due/overdue reason |
| `NoopAgendaPort` | `SoulPorts.agenda` 默认 | 非 production | 测试/隔离：agenda 读恒空 |
| Candidate memory adapters | `memory/adapters/*` | 未接入 runtime | 忽略即等于禁用 |

> **P1-3 后续**：完整 flag inventory（含 longing/Poisson 参数、LLM budget）在单独 session 补表。
...
| 优先级 | 任务 | 说明 |
|---|---|---|
| **P1-3** | Feature flag inventory | 扩展 §5 为全量 `BudgetConfig` + env 开关表 |
