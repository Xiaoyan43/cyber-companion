# HANDOFF — 上下文交接（2026-06-27，第七十三轮 · master 基线干净集成）
...
2. **当前分支 = `codex/product-integration-20260627`，implementation checkpoint = `a0e23dc`**，直接基于最新 `origin/master@bd616d7`；独立 integration worktree 干净，不包含主工作区的 `_LatencySpikeLogger`、实验脚本、音频或 agent/MCP 配置。
3. 生效配置：
   - `config/providers.json`（本地 gitignored）：辅助 OpenRouter 条目已正名为 `"tagger"`，当前模型仍可用 `anthropic/claude-haiku-4.5`；新 env `OPENROUTER_TAGGER_API_KEY` 自动兼容旧 `OPENROUTER_GEMINI_API_KEY`。
   - `config/tts.json`（tracked）：默认已是 paid `s2.1-pro` + 嘉岚 `fbe02f8306fc4d3d915e9871722a39d5`。
   - `.env`（本地 gitignored）：推荐 `CYBER_COMPANION_VOICE_FISH_NORMALIZE=false`；未设则走 Pipecat 默认。
4. **语音/tagger 微调与分轨稳定化已结案**：master 已含 Fish Settings、S2.1 Pro、B 类中句位置与跨句去重；本分支补齐 provider 正名、移除 tagger 动态 mood 注入、单字尾音自回声窄修复。下一步转入产品体验，不再继续 Fish/tagger 微调。
5. 本分支门禁：backend **746 passed**、invariant **367 passed**、前端 `tsc --noEmit` 通过、`git diff --check` 通过。
...
**Soul Runtime 与已验收 voice checkpoint 已在最新 master 基线上完成干净集成。当前主线转为产品体验：把现有文本、语音、记忆、情绪和主动性收敛成可每天使用的完整闭环。**
...
- **`s2.1-pro` vs `s2.1-pro-free`**：用户 A/B 认为 paid 更好，repo 默认已切到 `s2.1-pro`。
- **`_LatencySpikeLogger`**：仅主 dirty 工作树本地探针，**不进生产 PR**。
- **~~标签器 provider 命名债~~ → 已完成**：默认条目为 `"tagger"`，并保留旧配置/env 兼容。
- **~~B 类位置提前染色~~ → 已完成**：动态 mood 不再注入 tagger；正文措辞是唯一情绪证据，`position_v5` 用户听感 PASS。
...
- 沿用：非干净后缀自回声（AEC epic）、P12、P9-P2-C、P9-D、日语音色未接后端、Fish WS 空闲断连。
...
目标：不再扩展底层能力，验证现有产品能否成为稳定的日常伴侣。

推荐最小顺序：

1. 固定一个默认入口与默认语音路径，避免用户面对多套 transport/实验开关。
2. 覆盖启动 → 文本/语音对话 → 记忆召回 → 情绪/关系反馈 → 关闭重开恢复的完整路径。
3. 记录真实使用摩擦，按阻塞频率排序；只有实际阻塞日用的问题才进入下一轮实现。
4. UI 方向仍未拍板，不启动大型视觉重做；只允许低 GPU、asset/CSS 型小步体验改进。

### 🟡 单字自回声后续观察

- 已补 2 秒精确尾字窄窗口与确定性测试；不全局吞掉真实单字回复。
- 日常真机若再次自触发，记录 ASR final 与相对 `BotStopped` 时间；不要主动制造回声或扩大模糊匹配。
...
非干净后缀自回声/AEC、P12、P9-P2-C、P9-D、日语音色切换。
...
- **产品体验**：`docs/PROJECT_BRIEF.md` + `docs/TODO.md` 的 Current Priority/Integration/Backlog；按一条端到端日用路径选文件，不做全仓扫描。
...
1. 完成本 integration 分支的 review/合入，不回放旧 Soul/voice 分支历史。
2. 启动“日用闭环 P0”：先做现状端到端验证和摩擦清单，不先加功能。
3. 根据真实使用结果选择一个最小修复；视觉、推送、联网等大项继续独立立项。
...
> 用户真机听感结论已记录并落地：`normalize=false` 推荐保留，paid `s2.1-pro` 已为 repo 默认，`position_v5` 已验收。下一步是日用闭环，不再继续 TTS 微调。
