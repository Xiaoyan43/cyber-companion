...
> Phase 2 completed at the SoulTurnRuntime boundary: ports are introduced with SQLite adapters; MemoryStore internals and schema are unchanged.
> 当前 `MemoryPort` 暂时偏宽，包含 motivation/context/budget 相关 runtime 边界方法；这是 Phase 2 接受的边界 adapter，不在本阶段返工拆分。
> **Phase 3B 已落地 `AgendaPort`**：实际签名为 `open_loops(*, status='open', due_before=None, limit=50)` / `upsert(OpenLoopDraft)` / `close(loop_id)`（下方 sketch 用 `now` 仅为示意）。`SoulPorts.agenda` 默认 `NoopAgendaPort`，`ports_from_store` 填 `SQLiteAgendaPort`；runtime 暂不读写 agenda。
